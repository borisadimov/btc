...
import {CLIENT_ID, CLIENT_SECRET, API_ENDPOINT, sendRequest, getQueryVariable} from '../serverApi';
...
export const REFERRAL_BONUS_REQUEST     = 'app/users/REFERRAL_BONUS_REQUEST';
export const REFERRAL_BONUS_RESPONSE    = 'app/users/REFERRAL_BONUS_RESPONSE';
...
export function requestReferralBonus() {
  return dispatch => {
    dispatch({
      type: REFERRAL_BONUS_REQUEST
    });

    let options = {method: 'POST'};
    sendRequest('/api/withdraw_bonuses', options)
      .then(res => {
        dispatch({
          type: REFERRAL_BONUS_RESPONSE
        });
        store.dispatch(getUserInfo());
      });
  };
  ...
  switch (action.type) {
    ...
    case GET_INFO_RESPONSE:
      return {
        ...
        referralBalance:    action.info.referral_balance    !== undefined   ?   math.bignumber(action.info.referral_balance)  : state.referralBalance,
    ...
    case REFERRAL_BONUS_REQUEST:
    case REFERRAL_BONUS_RESPONSE:
    default:
      return state;
  }
}